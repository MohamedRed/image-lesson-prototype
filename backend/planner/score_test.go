...
func TestResourceLedgerFromRawParsesFullyTypedNestedResourceMapEntries(t *testing.T) {
	ledger := resourceLedgerFromRaw([]any{
		map[string]map[string]int64{"items": {"suitcase": 2}},
		map[string]map[string]string{"items": {"duffel": " 1 ", "bad": "-2"}},
		map[string]map[string]float64{"items": {"crate": 3, "fractional": 1.5}},
	}, "items")
...
func TestResourceCountsFromRawParsesTypedStringResourceMaps(t *testing.T) {
	capacity := resourceCountsFromRaw(map[string]string{"suitcase": " 2 ", "duffel": "1", "bad": "-4"})
	if got := capacity["suitcase"]; got != 2 {
		t.Fatalf("expected typed string suitcase capacity to parse as 2, got %d from %#v", got, capacity)
	}
	if got := capacity["duffel"]; got != 1 {
		t.Fatalf("expected typed string duffel capacity to parse as 1, got %d from %#v", got, capacity)
	}
	if got := capacity["bad"]; got != 0 {
		t.Fatalf("expected negative typed string capacity entries to be ignored, got %d", got)
	}
}
...
func TestTransferPointLocationFromDataParsesStringBackedCoordinates(t *testing.T) {
	point, ok := transferPointLocationFromData(map[string]any{
		"geometry": map[string]any{
			"type":        "Point",
			"coordinates": []any{" -0.10 ", " 0.05 "},
		},
	})
	if !ok {
		t.Fatalf("expected string-backed transfer point coordinates to parse")
	}
	if point.Latitude != 0.05 || point.Longitude != -0.10 {
...
func TestTransferPointLocationFromDataParsesTypedFloatCoordinates(t *testing.T) {
	point, ok := transferPointLocationFromData(map[string]any{
		"geometry": map[string]any{
			"type":        "Point",
			"coordinates": []float64{-0.10, 0.05},
		},
	})
	if !ok {
		t.Fatalf("expected typed float transfer point coordinates to parse")
	}
	if point.Latitude != 0.05 || point.Longitude != -0.10 {
		t.Fatalf("expected transfer point lat=0.05 lon=-0.10, got %#v", point)
	}
...
func TestGeoJSONGeometryFromRawParsesTypedStringCoordinateRings(t *testing.T) {
	raw := map[string]any{
		"type": "Polygon",
		"coordinates": [][][]string{{
			{"-0.01", "-0.005"},
	...
	geometry, ok := geoJSONGeometryFromRaw(raw)
	if !ok || !validGeoJSONPolygonGeometry(geometry) {
		t.Fatalf("expected typed string raw routeBuffer coordinates to parse, got ok=%v geometry=%#v", ok, geometry)
	}
}
...
func TestGeoJSONGeometryFromRawParsesTypedStringMultiPolygonCoordinateRings(t *testing.T) {
	raw := map[string]any{
		"type": "MultiPolygon",
		"coordinates": [][][][]string{{{
			{"-1", "0"},
	...
	geometry, ok := geoJSONGeometryFromRaw(raw)
	if !ok || !validGeoJSONPolygonGeometry(geometry) {
		t.Fatalf("expected typed string raw MultiPolygon routeBuffer coordinates to parse, got ok=%v geometry=%#v", ok, geometry)
	}
}
...
func TestComputeDriverScore_WhitespaceOnlyGlobalWalkLimitDoesNotEnableExplicitBufferCap(t *testing.T) {
	t.Setenv("MAX_SINGLE_HOP_WALK_METERS", " 	 ")
	req := corridorRequest()
	req.OriDriveIso = GeoJSONGeometry{}
	req.OriWalkIso = rectPolygon(0.045, -0.01, 0.055, 0.01)
	req.DestWalkIso = rectPolygon(0.045, 0.99, 0.055, 1.01)
	driver := corridorDriver("buffer-only-blank-walk-limit", 0.05, 0, rectPolygon(0.045, -0.01, 0.055, 1.01))

	_, _, ok := computeDriverScore(req, driver, 1, 0.7, 0.3, 1)
	if !ok {
		t.Fatalf("expected whitespace-only global walk cap env to be treated as unset for legacy buffer-only corridor matching")
	}
}
...
func TestComputeDriverScore_IgnoresMalformedOptionalOriginDriveGeo(t *testing.T) {
	allowLongPickupETA(t)
	req := corridorRequest()
	req.OriDriveIso = GeoJSONGeometry{}
	req.OriginDriveGeo = GeoJSONGeometry{Type: "Polygon", Coordinates: [][][]float64{}}
	driver := corridorDriver("valid-corridor-malformed-origin-drive", 0, -0.10, GeoJSONGeometry{})
	driver.RoutePolyline = encodePolyline([]GeoPoint{
		{Latitude: 0, Longitude: -0.10},
		{Latitude: 0, Longitude: 0},
		{Latitude: 0, Longitude: 1},
	...
	_, _, ok := computeDriverScore(req, driver, 1, 0.7, 0.3, 1)
	if !ok {
		t.Fatalf("expected malformed optional originDriveGeo not to reject an otherwise valid corridor")
	}
}
...
func TestTransferCongestionFactorFromZoneDataMirrorsCurbNormalization(t *testing.T) {
	if got := transferCongestionFactorFromZoneData(map[string]any{"curbLoadFactor": " 1.25 "}); got != 1.25 {
		t.Fatalf("expected string-backed transfer curbLoadFactor to parse, got %f", got)
	}
	if got := transferCongestionFactorFromZoneData(map[string]any{"curbLoadFactor": int64(2)}); got != 2 {
		t.Fatalf("expected integer transfer curbLoadFactor to parse, got %f", got)
	}
	if got := transferCongestionFactorFromZoneData(map[string]any{"curbLoadFactor": math.Inf(1)}); got != 1 {
		t.Fatalf("expected non-finite transfer curbLoadFactor to fall back to neutral 1.0, got %f", got)
	}
}
